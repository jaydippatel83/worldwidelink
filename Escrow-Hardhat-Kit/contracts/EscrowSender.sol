...
contract EscrowSender is CCIPReceiver, OwnerIsCreator {
   ...
   function createTestSendPAYLINK(
       uint64 _destinationChainSelector,
       address _receiver,
       uint256 numOfAgree
       // string memory purpose
   ) internal returns (bytes32 messageId){
         Client.EVM2AnyMessage memory evm2AnyMessage = _buildTestCCIPMessage(
           _receiver,
           numOfAgree,
           // purpose,
           address(linkToken)
   ...
   function testPurpose(address _receiver) public {
       uint256 setAgreement = 103;


       createTestSendPAYLINK(
       ...
       override
   {
       ...
       (EscrowAgreement memory message, uint256 _numOfAgreement) = abi.decode(
           any2EvmMessage.data,
           (EscrowAgreement, uint256)
       );
   ...
   function _buildTestCCIPMessage(
       address _receiver,
       ...
       string memory purpose1 = "testPurpose";


        return
           Client.EVM2AnyMessage({
               receiver: abi.encode(_receiver),
               data: abi.encode(purpose1, _numOfAgreement ),
